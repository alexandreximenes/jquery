...
function setLocalStorage(data){
    let dados = [];
    dados.push(data);
    localStorage.setItem("frases", JSON.stringify(dados));
}

function getLocalStorage() {
    let dados = JSON.parse(localStorage.getItem("frases"));
    dados.forEach( (d) => console.log(d));
  return dados;
}

function fazendoExperiencias(data){
    console.clear();
    data.map(dados => {
            //Template String ECMA5
            console.log(`id: ${dados._id} \nTexto: ${dados.texto} \nTempo: ${dados.tempo}`);
        });

        // Somando o tempo total 
        var total = data.reduce((total, n) => total + n.tempo, 0);

        let resultado = "Tempo total : " + Math.floor(total / 60) + ":" + (total - 60) + " minutos"

        saidaColoridaNoConsole(resultado);

}

function saidaColoridaNoConsole(resultado){
    ...
    console.log("%cTotal: %s", css, resultado);
}